...
export default function CorrelationMatrix({
  ...
  if (
    ...
    return (
      <div className="correlation-container">
        <div className="error">
          Invalid correlation data format or no symbols available
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="correlation-container">
      ...
      <p className="correlation-description">
        Correlation values range from -1 to 1. Values closer to 1 indicate
        positive correlation, values closer to -1 indicate negative correlation,
        and values near 0 indicate no correlation.
      </p>
      ...
      <div className="correlation-matrix">
        <div className="matrix-header">
          ...
          {availableSymbols.map((symbol) => (
            <div key={symbol} className="header-cell">
              {symbol}
            </div>
          ))}
        </div>
        ...
        {availableSymbols.map((rowSymbol) => (
          <div key={rowSymbol} className="matrix-row">
            ...
            {availableSymbols.map((colSymbol) => {
              const corrValue = returns_correlation[rowSymbol]?.[colSymbol];
              const displayValue =
                corrValue !== undefined &&
                corrValue !== null &&
                !isNaN(corrValue)
              ...
              return (
                <div
                  ...
                  style={{
                    backgroundColor: getCorrelationColor(displayValue),
                    color: getTextColor(displayValue),
                  }}
                  title={`${rowSymbol} vs ${colSymbol}: ${displayValue.toFixed(
                    3
                  )}`}
                >
      ...
      <div className="correlation-stats">
        <p>
          ...
          {correlationData.analysis_date
            ? new Date(correlationData.analysis_date).toLocaleString()
            : "N/A"}
        </p>
        <p>
          <strong>Data Points:</strong> {correlationData.data_points || "N/A"}
        </p>
      </div>
    </div>
  );
